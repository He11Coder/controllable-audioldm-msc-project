    ...
    def _validate(self, step, start_time):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                _, _, fmap_s_r, fmap_s_g = self.msd(y, y_g_hat)
                ...
                total_gen_loss += (loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + 45*loss_mel).item()
        ...
        return avg_gen_loss, avg_disc_loss, avg_mel_loss
    ...
    def train(self):
        ...
        while step <= self.conf.steps:
            ...
            if step % self.conf.log_interval == 0:
                self._log_training(step, start_time, loss_gen_all, loss_disc_all, loss_mel)

            if step % self.conf.validation_interval == 0:
                val_gen_loss, val_disc_loss, val_mel_loss = self._validate(step, start_time)
